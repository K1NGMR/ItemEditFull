...
public class MoreExpansionAbilities implements Listener {
    ...
    private static final Map<UUID, Long> activeManaShields = new java.util.concurrent.ConcurrentHashMap<>();
    private static final Map<UUID, Long> activeLastStands = new java.util.concurrent.ConcurrentHashMap<>();
    private static final Map<UUID, Long> activeResurrections = new java.util.concurrent.ConcurrentHashMap<>();
    private static final Map<UUID, Long> activeDodges = new java.util.concurrent.ConcurrentHashMap<>();
    ...
    public static void register(ItemEditFull plugin) {
        ...
        plugin.getAbilityManager().registerAbility(new MeteorTierAbility(plugin, 1, "falling_star_strike", "Falling Star Strike", "Calls down a single blazing star that streaks from the sky in a shower of sparks."));
        plugin.getAbilityManager().registerAbility(new MeteorTierAbility(plugin, 2, "huge_meteorstrike", "Huge Meteor Strike", "Summons a huge molten meteor trailing flame and lava."));
        plugin.getAbilityManager().registerAbility(new MeteorTierAbility(plugin, 3, "massive_meteorstrike", "Massive Meteor Strike", "Summons a massive meteor whose shockwave ripples outward on impact."));
        plugin.getAbilityManager().registerAbility(new MeteorTierAbility(plugin, 4, "galaxy_meteorstrike", "Galaxy Meteor Strike", "Summons a devastating galaxy-sized meteor shower."));
    ...
    public void onQuit(org.bukkit.event.player.PlayerQuitEvent event) {
        ...
        activeManaShields.remove(id);
        activeLastStands.remove(id);
        activeResurrections.remove(id);
        activeDodges.remove(id);
    }
        ...
        if (event.getEntity() instanceof Player) {
            ...
            if (p.getHealth() - event.getFinalDamage() <= 0) {
                if (MoreExpansionAbilities.hasRes(p.getUniqueId())) {
                    ...
                    event.setCancelled(true);
                    // Clamp to (0, maxHealth] so a heal_percent > 1.0 (or <= 0) can't throw / kill.
                    double resMax = p.getMaxHealth();
                    p.setHealth(Math.max(1.0, Math.min(resMax, resMax * healPercent)));
            ...
            if (hasLastStandGear && p.getHealth() - event.getFinalDamage() <= p.getMaxHealth() * threshold) {
...
class WitherSkullBomb extends Ability {
    ...
    @Override public boolean trigger(Player p, ItemStack i) {
        ...
        EffectUtils.burst(p.getEyeLocation(), EffectUtils.layers(
                new Layer(Particle.SMOKE_LARGE, 12, 0.25, 0.25, 0.25, 0.02),
                new Layer(Particle.SOUL, 10, 0.2, 0.2, 0.2, 0.03)));
        EffectUtils.fanfare(p.getLocation(),
                new SoundLayer(Sound.ENTITY_WITHER_SHOOT, 1f, 1f),
                new SoundLayer(Sound.ENTITY_WITHER_AMBIENT, 0.6f, 1.3f));
        return true;
    }
}
class DragonBreathGround extends Ability {
    ...
    @Override public boolean trigger(Player p, ItemStack i) {
        ...
        EffectUtils.burst(loc, EffectUtils.layers(
                new Layer(Particle.DRAGON_BREATH, 30, 0.6, 0.2, 0.6, 0.05),
                new Layer(Particle.FLAME, 14, 0.4, 0.15, 0.4, 0.02)));
        EffectUtils.ring(loc, Math.max(1.0, radius * 0.6), 12, new Layer(Particle.DRAGON_BREATH, 2, 0.05, 0.05, 0.05, 0.01));
        p.getWorld().playSound(p.getLocation(), Sound.ENTITY_ENDER_DRAGON_GROWL, 1f, 1f);
        return true;
    }
}
class CreeperChargeAbility extends Ability {
    public CreeperChargeAbility(ItemEditFull pl) { super("creeper_supercharge", "Creeper Supercharge", "Grants supercharged explosion."); }
    @Override public boolean trigger(Player p, ItemStack i) {
        int duration = getIntParam(i, "duration", 200);
        int amp = getIntParam(i, "amplifier", 1);
        p.getWorld().strikeLightningEffect(p.getLocation());
        EffectUtils.burst(p.getLocation(), EffectUtils.layers(
                new Layer(Particle.FIREWORK_SPARK, 40, 0.4, 0.6, 0.4, 0.12),
                new Layer(Particle.EXPLOSION_NORMAL, 3, 0.3, 0.3, 0.3, 0)));
        EffectUtils.fanfare(p.getLocation(),
                new SoundLayer(Sound.ENTITY_CREEPER_PRIMED, 1.2f, 0.6f),
                new SoundLayer(Sound.ITEM_TRIDENT_THUNDER, 1f, 1.4f));
...
class CreeperDetonation extends Ability {
    ...
    @Override public boolean trigger(Player p, ItemStack i) {
        ...
        EffectUtils.burst(p.getLocation(), EffectUtils.layers(
                new Layer(Particle.SMOKE_NORMAL, 10, 0.3, 0.3, 0.3, 0.02),
                new Layer(Particle.FLASH, 1, 0, 0, 0, 0)));
        new CompatRunnable() {
            @Override public void run() {
                if (p.isOnline()) {
                    EffectUtils.burst(p.getLocation(), EffectUtils.layers(
                            new Layer(Particle.EXPLOSION_LARGE, 4, 0.4, 0.4, 0.4, 0),
                            new Layer(Particle.SMOKE_LARGE, 25, 0.6, 0.6, 0.6, 0.05)));
    ...
    @Override public boolean trigger(Player p, ItemStack i) {
        ...
        for (Entity vic : p.getWorld().getNearbyEntities(p.getLocation(), range, height, range)) {
            if (vic instanceof LivingEntity && !vic.equals(p)) {
                ...
                EffectUtils.burst(vic.getLocation(), EffectUtils.layers(
                        new Layer(Particle.END_ROD, 10, 0.3, 0.5, 0.3, 0.04),
                        new Layer(Particle.PORTAL, 8, 0.3, 0.3, 0.3, 0.4)));
            }
        }
        EffectUtils.ring(p.getLocation(), range * 0.5, 16, new Layer(Particle.END_ROD, 2, 0.05, 0.05, 0.05, 0.01));
        p.getWorld().playSound(p.getLocation(), Sound.ENTITY_SHULKER_TELEPORT, 1f, 1f);
        return true;
    }
}
class WardenSonicClapAbility extends Ability {
    public WardenSonicClapAbility(ItemEditFull pl) { super("sonic_clap_wave", "Sonic Clap Wave", "Deafening sonic wave."); }
    @Override public boolean trigger(Player p, ItemStack i) {
        double radius = getDoubleParam(i, "radius", 8.0);
        Location loc = p.getLocation();
        EffectUtils.ring(loc, radius * 0.3, 16, new Layer(Particle.EXPLOSION_NORMAL, 1, 0.05, 0.05, 0.05, 0));
        EffectUtils.ring(loc, radius * 0.7, 20, new Layer(Particle.CRIT, 1, 0.05, 0.05, 0.05, 0.05));
        EffectUtils.burst(loc, EffectUtils.layers(new Layer(Particle.SONIC_BOOM, 1, 0, 0, 0, 0)));
        for (Entity ent : loc.getWorld().getNearbyEntities(loc, radius, radius * 0.5, radius)) {
            if (ent instanceof LivingEntity && !ent.equals(p)) {
                EffectUtils.knockback((LivingEntity) ent, loc, 1.6, 0.4);
            }
        }
        p.getWorld().playSound(loc, Sound.ENTITY_WARDEN_SONIC_BOOM, 1f, 0.8f);
        return true;
    }
}
class WardenSculkInfection extends Ability {
    ...
    @Override public boolean trigger(Player p, ItemStack i) {
        ...
        EffectUtils.fanfare(target,
                new SoundLayer(Sound.BLOCK_SCULK_SHRIEKER_SHRIEK, 1.2f, 1.0f),
                new SoundLayer(Sound.BLOCK_SCULK_SPREAD, 1.0f, 0.6f));
        EffectUtils.burst(target, EffectUtils.layers(
                new Layer(Particle.SPELL_WITCH, 40, 3.0, 0.5, 3.0, 0.05),
                new Layer(Particle.SOUL, 20, 2.5, 0.3, 2.5, 0.03),
                new Layer(Particle.SOUL_FIRE_FLAME, 10, 2.0, 0.2, 2.0, 0.01)));
        EffectUtils.ring(target, radius, 14, new Layer(Particle.SOUL, 2, 0.05, 0.05, 0.05, 0.01));
...
class EvokerFangCircle extends Ability {
    ...
    @Override public boolean trigger(Player p, ItemStack i) {
        ...
        EffectUtils.fanfare(base, new SoundLayer(Sound.ENTITY_EVOKER_CAST_SPELL, 1.2f, 0.9f));
        for (double d = 0; d < 360; d += angleStep) {
            ...
            EffectUtils.burst(loc, EffectUtils.layers(new Layer(Particle.CRIT_MAGIC, 6, 0.15, 0.1, 0.15, 0.05)));
        }
        return true;
    }
}
class EvokerVexSwarm extends Ability {
    ...
    @Override public boolean trigger(Player p, ItemStack i) {
        ...
        EffectUtils.fanfare(p.getLocation(), new SoundLayer(Sound.ENTITY_VEX_CHARGE, 1.2f, 1.0f));
        for (int k = 0; k < count; k++) {
            Location loc = p.getLocation().clone().add((Math.random() - 0.5) * 2, 0.5, (Math.random() - 0.5) * 2);
            EffectUtils.burst(loc, EffectUtils.layers(
                    new Layer(Particle.PORTAL, 12, 0.3, 0.3, 0.3, 0.3),
                    new Layer(Particle.SPELL_WITCH, 6, 0.2, 0.2, 0.2, 0.05)));
            p.getWorld().spawn(loc, Vex.class);
        }
        return true;
    }
}
class WitchElixir extends Ability {
    ...
    @Override public boolean trigger(Player p, ItemStack i) {
        ...
        EffectUtils.burst(p.getLocation(), EffectUtils.layers(
                new Layer(Particle.SPELL_WITCH, 20, 0.3, 0.6, 0.3, 0.05),
                new Layer(Particle.VILLAGER_HAPPY, 10, 0.3, 0.5, 0.3, 0.02)));
...
class WitchPoisonSplash extends Ability {
    ...
    @Override public boolean trigger(Player p, ItemStack i) {
        ...
        int amp = getIntParam(i, "amplifier", 1);
        EffectUtils.burst(p.getEyeLocation(), EffectUtils.layers(
                new Layer(Particle.SPELL_WITCH, 14, 0.2, 0.2, 0.2, 0.04)));
        p.getWorld().playSound(p.getLocation(), Sound.ENTITY_WITCH_THROW, 1f, 1.1f);
...
class HuskDesiccation extends Ability {
    ...
    @Override public boolean trigger(Player p, ItemStack i) {
        ...
        EffectUtils.burst(loc, EffectUtils.layers(
                new Layer(Particle.DAMAGE_INDICATOR, 15, 3.0, 1.0, 3.0, 0.1),
                new Layer(Particle.SOUL, 12, 2.5, 0.6, 2.5, 0.03)));
        EffectUtils.ring(loc, radius, 12, new Layer(Particle.SMOKE_NORMAL, 1, 0.05, 0.1, 0.05, 0.01));
...
class MagmaCubeSlam extends Ability {
    ...
    @Override public boolean trigger(Player p, ItemStack i) {
        ...
        EffectUtils.burst(p.getLocation(), EffectUtils.layers(
                new Layer(Particle.LAVA, 10, 0.3, 0.1, 0.3, 0),
                new Layer(Particle.FLAME, 14, 0.3, 0.1, 0.3, 0.02)));
        p.getWorld().playSound(p.getLocation(), Sound.ENTITY_MAGMA_CUBE_JUMP, 1.2f, 0.9f);
...
class BlazeInferno extends Ability {
    ...
    @Override public boolean trigger(Player p, ItemStack i) {
        ...
        Location base = p.getLocation();
        EffectUtils.burst(base.clone().add(0, 1, 0), EffectUtils.layers(
                new Layer(Particle.FLAME, 30, 0.4, 0.4, 0.4, 0.05),
                new Layer(Particle.LAVA, 6, 0.2, 0.2, 0.2, 0)));
        p.getWorld().playSound(base, Sound.ENTITY_BLAZE_SHOOT, 1.2f, 0.9f);
        for (int k = 0; k < count; k++) {
            double angle = (2 * Math.PI * k) / count;
            Location spark = base.clone().add(Math.cos(angle) * 1.2, 1, Math.sin(angle) * 1.2);
            EffectUtils.burst(spark, EffectUtils.layers(new Layer(Particle.FLAME, 4, 0.05, 0.05, 0.05, 0.01)));
    ...
    @Override public boolean trigger(Player p, ItemStack i) {
        ...
        new CompatRunnable() {
            ...
            public void run() {
                ...
                EffectUtils.burst(p.getEyeLocation(), EffectUtils.layers(new Layer(Particle.VILLAGER_HAPPY, 4, 0.1, 0.1, 0.1, 0.03)));
    ...
    @Override public boolean trigger(Player p, ItemStack i) {
        ...
        new CompatRunnable() {
            ...
            public void run() {
                ...
                if (ticks % 4 == 0) {
                    EffectUtils.ring(target, radius * (1.0 - (ticks % 20) / 20.0), 10, new Layer(Particle.CLOUD, 1, 0.05, 0.05, 0.05, 0.01));
                }
                for (Entity ent : target.getWorld().getNearbyEntities(target, radius, height, radius)) {
                    if (ent instanceof LivingEntity && !ent.equals(p)) {
                        Vector pullDir = com.itemedit.full.utils.VectorUtils.safeNormalize(target.toVector().subtract(ent.getLocation().toVector())).multiply(pullForce).setY(0.05);
    ...
    @Override public boolean trigger(Player p, ItemStack i) {
        ...
        new CompatRunnable() {
            ...
            public void run() {
                ...
                Vector direction = com.itemedit.full.utils.VectorUtils.safeNormalize(tLoc.toVector().subtract(pEye.toVector()));
...
class ElderGuardianFatigueBlast extends Ability {
    ...
    @Override public boolean trigger(Player p, ItemStack i) {
        ...
        EffectUtils.ring(loc, radius, 18, new Layer(Particle.WATER_SPLASH, 3, 0.1, 0.3, 0.1, 0.02));
        EffectUtils.burst(loc, EffectUtils.layers(new Layer(Particle.CRIT_MAGIC, 20, 4.0, 1.0, 4.0, 0.05)));
        for (Entity ent : p.getWorld().getNearbyEntities(loc, radius, height, radius)) {
            if (ent instanceof LivingEntity && !ent.equals(p)) {
                LivingEntity le = (LivingEntity) ent;
                le.addPotionEffect(new PotionEffect(PotionEffectType.SLOW, slowDur, slowAmp));
...
class RavagerCharge extends Ability {
    ...
    @Override public boolean trigger(Player p, ItemStack i) {
        ...
        EffectUtils.burst(p.getLocation(), EffectUtils.layers(
                new Layer(Particle.CLOUD, 16, 0.4, 0.1, 0.4, 0.02),
                new Layer(Particle.CRIT, 10, 0.3, 0.1, 0.3, 0.05)));
        p.getWorld().playSound(p.getLocation(), Sound.ENTITY_RAVAGER_STEP, 1.2f, 0.7f);
...
class RavagerRoar extends Ability {
    ...
    @Override public boolean trigger(Player p, ItemStack i) {
        Location loc = p.getLocation();
        EffectUtils.ring(loc, 3.0, 16, new Layer(Particle.CLOUD, 2, 0.05, 0.1, 0.05, 0.02));
        EffectUtils.burst(loc.clone().add(0, 1, 0), EffectUtils.layers(new Layer(Particle.EXPLOSION_NORMAL, 2, 0.3, 0.2, 0.3, 0)));
        for (Entity ent : loc.getWorld().getNearbyEntities(loc, 6.0, 3.0, 6.0)) {
            if (ent instanceof LivingEntity && !ent.equals(p)) {
                EffectUtils.knockback((LivingEntity) ent, loc, 1.2, 0.3);
            }
        }
        EffectUtils.fanfare(loc, new SoundLayer(Sound.ENTITY_RAVAGER_ROAR, 1.5f, 1f));
        return true;
    }
}
...
class SlimeBounceAbility extends Ability {
    private final ItemEditFull plugin;
    public SlimeBounceAbility(ItemEditFull pl) { super("slime_safefall", "Slime Safe Fall", "Safe fall bounce."); this.plugin = pl; }
    @Override public boolean trigger(Player p, ItemStack i) {
...
class FireMeteor extends Ability {
    ...
    @Override public boolean trigger(Player p, ItemStack i) {
        ...
        new CompatRunnable() {
            ...
            Vector dir = com.itemedit.full.utils.VectorUtils.safeNormalize(target.toVector().subtract(sky.toVector())).multiply(0.5);
            @Override
            public void run() {
...
class WindPushAbility extends Ability {
    ...
    @Override public boolean trigger(Player p, ItemStack i) {
        ...
        for (Entity ent : p.getWorld().getNearbyEntities(loc, radius, height, radius)) {
            if (ent instanceof LivingEntity && !ent.equals(p)) {
                LivingEntity le = (LivingEntity) ent;
                Vector toTarget = com.itemedit.full.utils.VectorUtils.safeNormalize(le.getLocation().toVector().subtract(loc.toVector()));
...
class WindPull extends Ability {
    ...
    @Override public boolean trigger(Player p, ItemStack i) {
        ...
        for (Entity ent : target.getWorld().getNearbyEntities(target, radius, height, radius)) {
            if (ent instanceof LivingEntity && !ent.equals(p)) {
                Vector dir = com.itemedit.full.utils.VectorUtils.safeNormalize(target.toVector().subtract(ent.getLocation().toVector())).multiply(velocityMultiplier).setY(velocityY);
                ent.setVelocity(dir);
            }
        }
        return true;
        ...
        new CompatRunnable() {
            ...
            public void run() {
                ...
                for (Entity ent : target.getWorld().getNearbyEntities(target, pullRadius, pullHeight, pullRadius)) {
                    if (ent instanceof LivingEntity && !ent.equals(p)) {
                        Vector pull = com.itemedit.full.utils.VectorUtils.safeNormalize(target.toVector().subtract(ent.getLocation().toVector())).multiply(pullSpeed).setY(pullY);
    ...
    @Override public boolean trigger(Player p, ItemStack i) {
        ...
        new CompatRunnable() {
            ...
            public void run() {
                ...
                for (Entity ent : target.getWorld().getNearbyEntities(target, 5.0, 3.0, 5.0)) {
                    if (ent instanceof LivingEntity && !ent.equals(p)) {
                        Vector pull = com.itemedit.full.utils.VectorUtils.safeNormalize(target.toVector().subtract(ent.getLocation().toVector())).multiply(0.4).setY(0.05);
                        ent.setVelocity(pull);
                    }
                }
            }
...
class BrawlerTackle extends Ability {
    ...
    @Override public boolean trigger(Player p, ItemStack i) {
        ...
        if (target == null) return false;
        Vector dir = com.itemedit.full.utils.VectorUtils.safeNormalize(target.getLocation().toVector().subtract(p.getLocation().toVector()));
...
class BrawlerShockwave extends Ability {
    ...
    @Override public boolean trigger(Player p, ItemStack i) {
        ...
        for (Entity ent : p.getWorld().getNearbyEntities(loc, radius, height, radius)) {
            if (ent instanceof LivingEntity && !ent.equals(p)) {
                LivingEntity le = (LivingEntity) ent;
                Vector push = com.itemedit.full.utils.VectorUtils.safeNormalize(le.getLocation().toVector().subtract(loc.toVector())).multiply(pushMultiplier).setY(pushY);
                le.setVelocity(push);
                le.damage(damage, p);
                hit = true;
            }
...
class GrapplingHook extends Ability {
    ...
    @Override public boolean trigger(Player p, ItemStack i) {
        ...
        Vector dir = com.itemedit.full.utils.VectorUtils.safeNormalize(target.getLocation().toVector().subtract(p.getLocation().toVector())).multiply(velocityMultiplier).setY(velocityY);
        p.setVelocity(dir);
...
class TimeLeap extends Ability {
    private static final Map<UUID, List<Location>> history = new java.util.concurrent.ConcurrentHashMap<>();
    private final ItemEditFull plugin;
...
class SonicBoomAbility extends Ability {
    private final ItemEditFull plugin;
    public SonicBoomAbility(ItemEditFull pl) { super("sonic_wave", "Sonic Wave", "Fires a sonic boom wave."); this.plugin = pl; }
    @Override public boolean trigger(Player p, ItemStack item) {
        int count = getIntParam(plugin, item, "count", 1);
        double range = getDoubleParam(plugin, item, "range", 20.0);
        double damage = getDoubleParam(plugin, item, "damage", 10.0);
...
class SummonFriendAbility extends Ability {
    ...
    @Override public boolean trigger(Player p, ItemStack item) {
        ...
        // Guard against a configured type that isn't spawnable / has no entity class (would throw).
        if (!type.isSpawnable() || type.getEntityClass() == null || !LivingEntity.class.isAssignableFrom(type.getEntityClass())) {
            type = EntityType.PIG;
        }
        ...
        for (int i = 0; i < count; i++) {
            ...
            Entity ent;
            try {
                ent = p.getWorld().spawn(loc, finalType.getEntityClass());
            } catch (Exception e) {
                continue;
            }
                ...
                if (t >= fallTicks) {
                    ...
                    for (LivingEntity living : world.getLivingEntities()) {
                        if (!living.equals(p) && !living.isDead() && living.getLocation().distanceSquared(impactLoc) <= radiusSq) {
                            living.damage(damage, p);
                            if (knockback != 0.0) {
                                Vector kb = com.itemedit.full.utils.VectorUtils.safeNormalize(living.getLocation().toVector().subtract(impactLoc.toVector())).multiply(knockback).setY(0.4);
                                living.setVelocity(living.getVelocity().add(kb));
                            }
                        }
                    }
            ...
            @Override public void run() {
                ...
                for (FallingBlock fb : fallingBlocks) {
                    if (fb.isValid()) {
                        ...
                        Vector toTarget = com.itemedit.full.utils.VectorUtils.safeNormalize(targetLoc.toVector().subtract(fb.getLocation().toVector()));
    ...
    @Override public boolean trigger(Player p, ItemStack item) {
        ...
        new CompatRunnable() {
            @Override public void run() {
                ...
                final Vector travelDir = com.itemedit.full.utils.VectorUtils.safeNormalize(impact.toVector().subtract(orbLoc.toVector()));
                        ...
                        if (step >= steps || !orbLoc.getWorld().equals(impact.getWorld())) {
                            ...
                            for (Entity e : world.getNearbyEntities(impact, radius, radius, radius)) {
                                if (e instanceof LivingEntity && !e.equals(p)) {
                                    ...
                                    le.damage(damage, p);
                                    Vector push = com.itemedit.full.utils.VectorUtils.safeNormalize(le.getLocation().toVector().subtract(impact.toVector())).multiply(3.0).setY(1.2);
                                    le.setVelocity(push);
                                }
                            }
                                    ...
                                    for (int z = -r; z <= r; z++) {
                                        if (x * x + z * z <= r * r && Math.random() < 0.25) {
                                            ...
                                            if (b.getType() != Material.AIR && b.getType() != Material.BEDROCK) {
                                                ...
                                                fb.setDropItem(false);
                                                fb.setHurtEntities(false);
                                                Vector blockDir = com.itemedit.full.utils.VectorUtils.safeNormalize(fb.getLocation().toVector().subtract(impact.toVector()));
...
class VenomSpitAbility extends Ability {
      private final ItemEditFull plugin;
      public VenomSpitAbility(ItemEditFull pl) { super("venom_spray", "Venom Spray", "Spits toxic venom."); this.plugin = pl; }
      @Override public boolean trigger(Player p, ItemStack item) {
  ...
  class ArachnidJumpAbility extends Ability {
      private final ItemEditFull plugin;
      public ArachnidJumpAbility(ItemEditFull pl) { super("spider_leap", "Spider Leap", "Leap forward like a spider."); this.plugin = pl; }
      @Override public boolean trigger(Player p, ItemStack item) {
  ...
  /**
   * Shared engine for the Falling Star -> Huge -> Massive -> Galaxy meteor tier family.
   * Previously each tier was a hand copy-pasted class; consolidated here so the only thing
   * that differs between tiers is their numeric defaults and particle theme (kept visually
   * distinct per tier: star sparkle -> molten -> molten+shockwave -> cosmic swarm).
   */
  class MeteorTierAbility extends Ability {
      private final ItemEditFull plugin;
      private final int tier;

      public MeteorTierAbility(ItemEditFull pl, int tier, String id, String name, String description) {
      ...
      @Override public boolean trigger(Player p, ItemStack item) {
          switch (tier) {
              case 1: return triggerSingle(p, item, 15.0, 20, 20.0, 15.0, 1.2, 4.0, 6.0, 100, 2.0, 0.8, 60,
                      com.itemedit.full.utils.EffectUtils.layers(
                              new com.itemedit.full.utils.EffectUtils.Layer(Particle.FIREWORK_SPARK, 10, 0.2, 0.2, 0.2, 0.08),
                              new com.itemedit.full.utils.EffectUtils.Layer(Particle.CRIT_MAGIC, 6, 0.15, 0.15, 0.15, 0.1)),
                      com.itemedit.full.utils.EffectUtils.layers(
                              new com.itemedit.full.utils.EffectUtils.Layer(Particle.FIREWORK_SPARK, 60, 1.5, 1.5, 1.5, 0.15),
                              new com.itemedit.full.utils.EffectUtils.Layer(Particle.FLASH, 1, 0, 0, 0, 0)),
                      new com.itemedit.full.utils.EffectUtils.SoundLayer(Sound.ENTITY_FIREWORK_ROCKET_LAUNCH, 1.5f, 0.8f),
                      new com.itemedit.full.utils.EffectUtils.SoundLayer(Sound.ENTITY_FIREWORK_ROCKET_TWINKLE, 1.5f, 1.0f),
                      new com.itemedit.full.utils.EffectUtils.SoundLayer(Sound.ENTITY_GHAST_SHOOT, 1.0f, 1.0f));
              case 2: return triggerSingle(p, item, 30.0, 25, 60.0, 40.0, 1.5, 8.0, 10.0, 200, 3.0, 1.0, 100,
                      com.itemedit.full.utils.EffectUtils.layers(
                              new com.itemedit.full.utils.EffectUtils.Layer(Particle.FLAME, 10, 0.2, 0.2, 0.2, 0.1),
                              new com.itemedit.full.utils.EffectUtils.Layer(Particle.LAVA, 5, 0.1, 0.1, 0.1, 0)),
                      com.itemedit.full.utils.EffectUtils.layers(
                              new com.itemedit.full.utils.EffectUtils.Layer(Particle.EXPLOSION_LARGE, 5, 0, 0, 0, 0)),
                      new com.itemedit.full.utils.EffectUtils.SoundLayer(Sound.ENTITY_ENDER_DRAGON_GROWL, 2f, 0.5f), null, null);
              case 3: return triggerSingle(p, item, 100.0, 35, 140.0, 100.0, 2.0, 20.0, 25.0, 400, 5.0, 2.0, 200,
                      com.itemedit.full.utils.EffectUtils.layers(
                              new com.itemedit.full.utils.EffectUtils.Layer(Particle.FLAME, 30, 0.5, 0.5, 0.5, 0.2),
                              new com.itemedit.full.utils.EffectUtils.Layer(Particle.LAVA, 15, 0.3, 0.3, 0.3, 0),
                              new com.itemedit.full.utils.EffectUtils.Layer(Particle.EXPLOSION_NORMAL, 2, 0.1, 0.1, 0.1, 0)),
                      com.itemedit.full.utils.EffectUtils.layers(
                              new com.itemedit.full.utils.EffectUtils.Layer(Particle.EXPLOSION_HUGE, 20, 3, 3, 3, 0)),
                      new com.itemedit.full.utils.EffectUtils.SoundLayer(Sound.ENTITY_ENDER_DRAGON_GROWL, 5f, 0.2f), null, null);
              case 4: return triggerGalaxy(p, item);
              default: return false;
          }
      }

      private boolean triggerSingle(Player p, ItemStack item, double dDamage, int dRange, double dOffsetXZ,
              double dSkyOffset, double dSpeed, double dYield, double dRadius, int dFireTicks, double dPushMult,
              double dPushY, int dMaxTicks, java.util.List<com.itemedit.full.utils.EffectUtils.Layer> trail,
              java.util.List<com.itemedit.full.utils.EffectUtils.Layer> impactBurst,
              com.itemedit.full.utils.EffectUtils.SoundLayer castSound1,
              com.itemedit.full.utils.EffectUtils.SoundLayer castSound2,
              com.itemedit.full.utils.EffectUtils.SoundLayer castSound3) {
          double damage = getDoubleParam(plugin, item, "damage", dDamage);
          int range = getIntParam(plugin, item, "range", dRange);
          double offsetXZ = getDoubleParam(plugin, item, "offset-xz", dOffsetXZ);
          double skyOffset = getDoubleParam(plugin, item, "sky-offset", dSkyOffset);
          double speed = getDoubleParam(plugin, item, "speed", dSpeed);
          double yield = getDoubleParam(plugin, item, "yield", dYield);
          double radius = getDoubleParam(plugin, item, "radius", dRadius);
          int fireTicks = getIntParam(plugin, item, "fire-ticks", dFireTicks);
          double pushMultiplier = getDoubleParam(plugin, item, "push-multiplier", dPushMult);
          double pushY = getDoubleParam(plugin, item, "push-y", dPushY);
          int maxDurationTicks = getIntParam(plugin, item, "max-duration-ticks", dMaxTicks);

          Block target = p.getTargetBlock(null, range);
          Location impactLoc = target.getLocation();
          Location startLoc = com.itemedit.full.utils.EffectUtils.jitterXZ(impactLoc, offsetXZ).add(0, skyOffset, 0);
          if (castSound1 != null) p.getWorld().playSound(p.getLocation(), castSound1.sound, castSound1.volume, castSound1.pitch);
          if (castSound2 != null) p.getWorld().playSound(p.getLocation(), castSound2.sound, castSound2.volume, castSound2.pitch);
          if (castSound3 != null) p.getWorld().playSound(p.getLocation(), castSound3.sound, castSound3.volume, castSound3.pitch);

          com.itemedit.full.utils.EffectUtils.fallingTrail(plugin, p, startLoc, impactLoc, speed, maxDurationTicks, trail, () -> {
              impactLoc.getWorld().createExplosion(impactLoc, (float) yield, false, false);
              com.itemedit.full.utils.EffectUtils.burst(impactLoc, impactBurst);
              for (Entity e : impactLoc.getWorld().getNearbyEntities(impactLoc, radius, radius, radius)) {
                  if (e instanceof LivingEntity && !e.equals(p)) {
                      LivingEntity le = (LivingEntity) e;
                      le.damage(damage, p);
                      le.setFireTicks(fireTicks);
                      com.itemedit.full.utils.EffectUtils.knockback(le, impactLoc, pushMultiplier, pushY);
                  }
              }
          });
          return true;
      ...
      private boolean triggerGalaxy(Player p, ItemStack item) {
          ...
          Block target = p.getTargetBlock(null, range);
          Location impactLoc = target.getLocation();
          com.itemedit.full.utils.EffectUtils.fanfare(p.getLocation(),
                  new com.itemedit.full.utils.EffectUtils.SoundLayer(Sound.UI_TOAST_CHALLENGE_COMPLETE, 5f, 0.5f),
                  new com.itemedit.full.utils.EffectUtils.SoundLayer(Sound.ENTITY_ENDER_DRAGON_GROWL, 5f, 0.1f));

          com.itemedit.full.utils.EffectUtils.staggered(plugin, p, meteorCount, delayMultiplier, index -> {
              if (!p.isOnline()) return;
              Location individualImpact = com.itemedit.full.utils.EffectUtils.jitterXZ(impactLoc, impactOffsetXZ);
              Location startLoc = com.itemedit.full.utils.EffectUtils.jitterXZ(individualImpact, offsetXZ).add(0, skyOffset, 0);
              boolean hero = index == 0;

              java.util.List<com.itemedit.full.utils.EffectUtils.Layer> trail = com.itemedit.full.utils.EffectUtils.layers(
                      new com.itemedit.full.utils.EffectUtils.Layer(Particle.PORTAL, hero ? 70 : 45, 0.8, 0.8, 0.8, 0.15),
                      new com.itemedit.full.utils.EffectUtils.Layer(Particle.DRAGON_BREATH, hero ? 16 : 10, 0.3, 0.3, 0.3, 0.05, 1.0f),
                      new com.itemedit.full.utils.EffectUtils.Layer(Particle.END_ROD, hero ? 12 : 8, 0.2, 0.2, 0.2, 0.05));

              com.itemedit.full.utils.EffectUtils.fallingTrail(plugin, p, startLoc, individualImpact,
                      hero ? 2.5 : 2.0, maxDurationTicks, trail, () -> {
                  individualImpact.getWorld().createExplosion(individualImpact, hero ? 35f : 15f, false, false);
                  com.itemedit.full.utils.EffectUtils.burst(individualImpact, com.itemedit.full.utils.EffectUtils.layers(
                          new com.itemedit.full.utils.EffectUtils.Layer(Particle.EXPLOSION_HUGE, hero ? 40 : 15, 2, 2, 2, 0),
                          new com.itemedit.full.utils.EffectUtils.Layer(Particle.DRAGON_BREATH, hero ? 150 : 100, 3.0, 3.0, 3.0, 0.2, 1.0f)));
                  individualImpact.getWorld().playSound(individualImpact, Sound.ENTITY_DRAGON_FIREBALL_EXPLODE, 10f, 0.5f);

                  double radius = hero ? 35.0 : 15.0;
                  for (Entity e : individualImpact.getWorld().getNearbyEntities(individualImpact, radius, radius, radius)) {
                      if (e instanceof LivingEntity && !e.equals(p)) {
                          LivingEntity le = (LivingEntity) e;
                          le.damage(damage / (hero ? 1.0 : 2.0), p);
                          le.addPotionEffect(new PotionEffect(PotionEffectType.WITHER, witherDuration, witherAmplifier));
                          com.itemedit.full.utils.EffectUtils.knockback(le, individualImpact, hero ? 8.0 : 4.0, 1.5);
                      }
                  }
              });
          });
